...
![INSERT CHART: Annualized Returns by Decile](placeholder_path)

**Honest noise-vs-signal read.** The most likely explanation for the negative long-short spread is the mega-cap tilt disclosed above (raw breadth change mechanically favors higher-baseline-breadth names, most visible in the short leg) colliding with an 11-year window that was, in aggregate, a strong and increasingly mega-cap-concentrated bull market. Being structurally short a mega-cap-tilted basket for most of 2015-2026 would produce exactly this signature — a wide, volatile, underperforming spread — largely independent of whether breadth momentum carries genuine security-level information. The rank-IC result is consistent with that read: a small, non-significant positive tilt survives at the cross-sectional level (where the mega-cap confound is diluted across the whole scored universe), while the decile-portfolio construction — which concentrates the confound in one leg — shows a much worse result. **Confidence is low that this specific construction reflects a tradeable edge; confidence is also low that the underlying breadth-momentum concept is worthless.** The evidence is too weak in either direction to conclude cleanly — that is the honest conclusion, not a hedge.
...
2. **Test a scale-neutralized signal variant** as a targeted robustness check, not a wholesale redesign: a percentage- or baseline-breadth-adjusted version of the raw change, run through the identical backtest machinery, to test whether removing the mega-cap tilt from the short leg closes the gap between the weakly-positive IC and the negative portfolio result. This is the most direct test of this memo's central open question.
